...
def layer_selection_info(layer: Layer) -> dict[str, list[int]|None|bool|int]:
    ...
    selection_OIDs: list[int]|None = Describe(layer)['FIDSet']
    has_selection: bool = True if selection_OIDs else False
    selection_count: int = len(selection_OIDs) if has_selection else 0

    return {'selection_OIDs': selection_OIDs, 'has_selection': has_selection, 'selection_count': selection_count}
