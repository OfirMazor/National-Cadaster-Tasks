...
ENV.workspace = fr"{CNFG.ParcelFabricFeatureServer};version={get_VersionName('רישומים')}"
...
def modify_ParcelsAttributes(ProcessName: str) -> None:
    ...
    source_parcels: Scur = SearchCursor(get_layer('חלקות ביסוס'), ['ParcelNumber', 'BlockNumber', 'SubBlockNumber', 'LandDesignationPlan'])
    total: int = cursor_length(source_parcels)

    AddMessage('\n ⭕ Modifying parcels attributes:')

    for idx, row in enumerate(source_parcels, start=1):
        ParcelNumber, BlockNumber, SubBlockNumber = row[0], row[1], row[2]
        NewLandDesignation: str|None = row[3] if row[3] not in ['None', 'none', ''] else None
    ...
    del Parcels2D, Ucursor, RecordGUID, source_parcels


def modify_CurrentFrontsAttributes() -> None:
    """
    ...
    for idx, guid in enumerate(process_fronts_guids, start=1):
        process_front: Layer = MakeLayer(process_fronts_layer, 'process_front', where_clause = f"GlobalID = '{guid}'")[0]
        ...
        count_matches: int = int(current_front[2])
        ...
        if count_matches == 1:
            ...
            current_data = UpdateCursor(current_front[0], fields_to_update)
            for row in current_data:
                ...
                row[4]: str|None   = get_StartPointGUID(row[6])
                row[5]: str|None   = get_EndPointGUID(row[6])
    ...
    del current_map, fields_to_update, current_fronts_layer, total_unmatched
...
def modify_CurrentAndNewFrontsAttributes() -> None:
    ...
    for idx, guid in enumerate(process_fronts_guids, start=1):
        ...
        if count_matches == 1:
            ...
            current_data = UpdateCursor(current_front[0], ['GlobalID', 'Distance', 'LineType', 'Radius', 'Shape', 'StartPointUniqueID', 'EndPointUniqueID', 'Shape@'])
            for row in current_data:
    ...
    del current_map, current_fronts_layer, count_matches, process_fronts_layer, process_fronts_guids, total
...
def modify_PointsAttributes(ProcessName: str, task: TaskType) -> None:
    ...
    active_points_layer: Layer = current_map.listLayers('נקודות גבול')[0]
    selection_params: dict[str, Any] = {'in_layer': active_points_layer, 'select_features': process_points_layer, 'selection_type': 'NEW_SELECTION', 'overlap_type': 'ARE_IDENTICAL_TO'}
    active_points_layer: Layer = SelectByLocation(**selection_params)[0]

    if not compare_counts(process_points_layer, active_points_layer):
        AddMessage(' ⚠️ Warning: Not all points are matched')


    AddMessage('\n ⭕ Modifying points attributes: \n')
    process_points_names: Scur = SearchCursor(process_points_layer, ['PointName', 'OBJECTID'])
    total: int = cursor_length(process_points_names)
    for idx, process_point in enumerate(process_points_names, start=1):
        source_name: str = process_point[0]
        source_oid: int = process_point[1]
        source_point_layer: Layer = SelectByAttribute(process_points_layer, where_clause=f'OBJECTID = {source_oid}')[0]

        selection_params: dict[str, Any] = {'in_layer': active_points_layer, 'select_features': source_point_layer, 'overlap_type': 'ARE_IDENTICAL_TO'}
        active_point: Layer = SelectByLocation(**selection_params)[0]
        current_name: str = SearchCursor(active_point, 'Name').next()[0]
        CalculateField(active_point, 'Name', expression = f"'{source_name}'", expression_type= "PYTHON3")
        AddMessage(f'{timestamp()} | {idx}|{total} | ✔️ Modifying point name from {current_name} to {source_name}')

        if task == 'ImproveCurrentCadaster':
            CalculateField(active_point, 'UpdatedByRecord', expression=f"'{RecordGUID}'", expression_type="PYTHON3")

        del source_name, source_oid, source_point_layer, active_point, current_name

    current_map.clearSelection()
    del RecordGUID, current_map, selection_params, active_points_layer, process_points_names, total
    return None
...
def modify_3DPointsAttributes(ProcessName) -> None:
    ...
    points_to_preserve: Layer = MakeLayer(fr"{CNFG.ParcelFabricDatabase}{CNFG.OwnerName}/InProcessBorderPoints3D", 'points_to_preserve', query)[0]
    data: Scur = SearchCursor(points_to_preserve, ['Name', 'Class', 'Shape'])
    total: int = cursor_length(data)

    active_points: Layer = SelectByLocation(get_layer('נקודות גבול תלת ממדיות'), 'ARE_IDENTICAL_TO', points_to_preserve)[0]
...
def modify_BlockAttributes(ProcessName: str) -> None:
    ...
    del Block_area, TotalArea, Blocks
...
def retire_fronts(ProcessName: str) -> None:
    ...
    ENV.addOutputsToMap = False
    substantiated_fronts: Layer = MakeLayer(InProcessFronts, 'InProcessFronts_layer', f"CPBUniqueID = '{get_ProcessGUID(ProcessName, 'MAP')}' AND LineStatus = 1")[0]
    ...
    if count_substantiated > 0:
        ...
        Fronts: Layer = SelectByLocation(in_layer = Fronts, overlap_type = "ARE_IDENTICAL_TO", select_features = substantiated_fronts, selection_type = 'NEW_SELECTION')[0]
        CalculateField(in_table= Fronts, expression_type = 'PYTHON3', field = 'RetiredByRecord', expression = f"'{RecordGUID}'")
        CurrentMap.clearSelection()

        unmatched_fronts: Layer = SelectByLocation(in_layer= substantiated_fronts, overlap_type= "ARE_IDENTICAL_TO", select_features= Fronts, selection_type= 'NEW_SELECTION', invert_spatial_relationship= 'INVERT')[0]
...
def retire_3D_points(ProcessName: str, tolerance: float = 0.002) -> None:
    ...
    if total > 0:
        ...
        CalculateField(in_table=to_retire[0], expression_type='PYTHON3', field='CancelProcessType', expression=cancel_process_type)
        CalculateField(in_table=to_retire[0], expression_type='PYTHON3', field='RetiredByRecord', expression=f"'{record_guid}'")
...
def reshape_transferring_block(ProcessName: str) -> None:
    ...
    if process_will_retire_its_block(ProcessName):
        #   No borders reshaping is needed if the block has retired.
        pass
